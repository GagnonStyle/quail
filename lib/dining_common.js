    ...
    if(err)
      ...
      pg.connect(constr, (err, client, done) => {
        if (err) {
          ...
          // add DC to database
          client.query('insert into dining_commons values (default,$1,$2,$3,$4)', [data.name, data.location, 1, 1], (err, result) => {
              done();
              if (err) {
                cb('could not connect to the database: ' + err);
                return;
...
// Change levels of dining common
function changeLevels(data, cb) {
  pg.connect(constr, (err, client, done) => {
    if (err) {
      cb('could not connect to the database: ' + err);
      return;
      ...
      // CHANGE LEVELS
      client.query('UPDATE "dining_commons" SET (noise_level, crowdedness) = ($2, $3) WHERE dcid = $1', [data.dcid, data.noiseLevel, data.trafficLevel], (err, result) => {
        done();
