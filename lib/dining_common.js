...
function changeLevels(noise, traffic, cb) {
  pg.connect(constr, (err, client, done) => {
    if (err) {
      cb('could not connect to the database: ' + err);
      return;
      ...
      // CHANGE LEVELS
      client.query('update dining_commons set (noise_level, crowdedness) = ($1, $2)', [noise, traffic], (err, result) => {
        done();
