...
function allTeamMembers(cb) {
  pg.connect(constr, (err, client, done) => {
    if (err) {
      cb('could not connect to the database: ' + err);
      return;
    }
    client.query('select * from users LIMIT 6', (err, result) => {
      done();
      ...
      var teamMembers = result.rows;

      cb(undefined, teamMembers);
    });
...
exports.allTeamMembers = allTeamMembers;
